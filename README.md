...
Local-first search agent built with LangGraph and Ollama. This project demonstrates an implementation of the ReAct (Reasoning + Acting) pattern, allowing a Large Language Model (LLM) to navigate the web autonomously to answer complex, real-time queries.

🏗️ Architecture & Design Decisions
Utilizes a State Machine architecture to handle the non-deterministic nature of AI agents.
...
### Prerequisites
- **Ollama** installed (see Ollama docs).  
- **Python 3.11+**
...
agentWithTools
Local agentic search using the ReAct pattern — a local-first, high-performance search agent built with LangGraph and Ollama. This project implements the ReAct pattern (Reasoning plus Acting) so an LLM can autonomously navigate web search and tool calls to answer complex, real-time queries.
...
[ ] Hybrid RAG — Integrate a local vector DB such as ChromaDB to combine web search with internal documentation
